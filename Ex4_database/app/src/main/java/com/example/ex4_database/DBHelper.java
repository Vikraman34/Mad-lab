...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public Boolean insertUserdata(String employee_code,String name,String gender,String deptartment,Double salary)
    {
    ...
    public Boolean updateUserdata(String employee_code,String name,Double salary)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        ContentValues cv=new ContentValues();
        cv.put("name",name);
        ...
        if(cursor.getCount()>0) {
            long result = db.update("Userdata", cv, "employee_code=?", new String[]{employee_code});
            if (result == -1)
                return false;
            else
                return true;
    ...
    public Boolean deleteUserdata(String employee_code)
    {
        ...
        Cursor cursor=db.rawQuery("SELECT * FROM Userdata WHERE employee_code=?",new String[] {employee_code});
        if(cursor.getCount()>0) {
            long result = db.delete("Userdata", "employee_code=?", new String[]{employee_code});
            if (result == -1)
                return false;
            else
                return true;
